...
void mexFunction (int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
  ...
  // get inliers
    ...
    if (nrhs!=1+0)
      mexErrMsgTxt("No input required.");
    if (nlhs!=1)
      mexErrMsgTxt("One output required (inliers).");

    // grab matches
    vector<int32_t> inliers = viso->getInlierIndices();

    // create output matrix with inliers
    const int32_t inliers_dims[] = {1,inliers.size()};
    plhs[0] = mxCreateNumericArray(2,inliers_dims,mxDOUBLE_CLASS,mxREAL);
    double* inliers_mex = (double*)mxGetPr(plhs[0]);

    // copy matches to mex array (convert indices: C++ => MATLAB)
    int32_t k=0;
    for (vector<int32_t>::iterator it=inliers.begin(); it!=inliers.end(); it++) {
      *(inliers_mex+k++) = double(*it);
    }
